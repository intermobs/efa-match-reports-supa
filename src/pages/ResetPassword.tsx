...
import { Eye, EyeOff, FileText, Lock, LogIn, ShieldCheck } from 'lucide-react';
...
export default function ResetPassword() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'Unable to update password. Please request a new reset link.';
      setError(message || 'Unable to update password. Please request a new reset link.');
    } finally {
        ...
        <section className="relative z-10 flex min-h-[420px] items-center rounded-2xl !bg-white px-6 py-10 sm:px-12 sm:py-14 lg:min-h-[600px]">
          <div className="mx-auto w-full max-w-[400px]">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              >
                {isSubmitting ? 'Updating password...' : <><LogIn size={20} /><span>Update password</span></>}
              </button>
            </form>

            <div className="mt-6 text-center text-sm">
              <Link to="/login" className="font-semibold text-blue-600 hover:underline">Back to sign in</Link>
            </div>
          </div>
        </section>
      </div>
